...
void Gather::eval_gpu(const std::vector<array>& inputs, array& out) {
  ...
  if (src.flags().row_contiguous && nidx == 1 && axes_[0] == 0 &&
    ...
    auto group_dims = get_block_dims(dim_x, dim_y, 1, kernel);
  ...
  auto group_dims = get_block_dims(dim0, dim1, dim2, kernel);
...
void GatherAxis::eval_gpu(const std::vector<array>& inputs, array& out) {
  ...
  auto group_dims =
      get_block_dims(size_post, idx_ax_size, size_pre, kernel);
